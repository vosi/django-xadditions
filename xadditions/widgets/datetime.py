...
class SelectTimeWidget(Widget):
    ...
    def __init__(self, attrs=None, hour_step=None, minute_step=None,
        ...
        (rather than 24-hr) use_seconds: If False, doesn't show seconds
        select element and stores seconds = 0.
    ...
    def render(self, name, value, attrs=None):
        ...
        if self.twelve_hr:
            if meridiem_val.lower().startswith('p') and \
        ...
        # For times to get displayed correctly, the values MUST be converted
        # to unicode When Select builds a list of options,
        ...
        if self.twelve_hr:
            ...
            select_html = Select(choices=meridiem_choices)\
                .render(self.meridiem_field % name, meridiem_val, local_attrs)
            output.append(select_html)
...
class SplitSelectDateTimeWidget(MultiWidget):
    ...
    This class combines SelectTimeWidget and SelectDateWidget so we have
    something like SpliteDateTimeWidget (in django.forms.widgets),
    but with Select elements.
    """
    def __init__(self, attrs=None, hour_step=None, minute_step=None,
                 second_step=None, twelve_hr=None, use_seconds=True,
                 required=True, years=None):
        """
